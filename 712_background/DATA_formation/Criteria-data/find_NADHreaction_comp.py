...
def main(argv):
        ...
        parser.add_option("-i", "--inputNADHreaction", action="store", type="string",
                dest="input_NADH", help='The input text file of reaction id list, which are related to NADH')
...
def make_file(input_NADH,root_dir):
        NADH_rn_name = read_file(input_NADH)
        output_file = "%s/NADHreaction_compoundPair.txt" %(root_dir)
        output = open(output_file, 'w')
 
	i = 0
	while i < len(NADH_rn_name):
        	rn_info = 'http://rest.kegg.jp/get/%s' % (NADH_rn_name[i])
        	...
        	for line in rn_INFO:
                	if re.search(r'\smain',line):
                        	...
                        	output.write('%s,%s,%s\n' % (NADH_rn_name[i], compound_split[0], compound_split[1]))
        	i += 1
	output.close()	
